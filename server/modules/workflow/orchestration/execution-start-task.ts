...
export function createExecutionStartTaskTools(deps: CreateExecutionStartTaskToolsDeps) {
  ...
  function parseGuildExecutionMeta(raw: string | null | undefined): { guildId: string; roleKey: string | null } | null {
    if (!raw) return null;
    try {
      const parsed = JSON.parse(raw) as Record<string, unknown>;
      const guildId = String(parsed.guildId ?? "").trim();
      const roleKey = String(parsed.currentGuildRole ?? parsed.roleKey ?? "").trim();
      return guildId ? { guildId, roleKey: roleKey || null } : null;
    } catch {
      return null;
    }
  }

  function findGuildRoleForRuntimeAgent(guildId: string, runtimeAgentId: string): string | null {
    const row = db
      .prepare(
        `SELECT r.role_key AS roleKey
         ...
         WHERE b.guild_id = ? AND b.runtime_agent_id = ?
         ORDER BY b.updated_at DESC
         LIMIT 1`,
      )
      .get(guildId, runtimeAgentId) as { roleKey: string } | undefined;
    return row?.roleKey ?? null;
  }

  function guildExecutionPreflight(taskId: string, requestedAgent: any):
    | { status: "ready"; agent: any; selectedBinding: GuildRuntimeBinding | null; switchedFrom: any | null }
    | { status: "blocked"; guildId: string; roleKey: string; selectedBinding: GuildRuntimeBinding | null; reason: string } {
    const taskMetaRow = db.prepare("SELECT workflow_meta_json FROM tasks WHERE id = ?").get(taskId) as
      | { workflow_meta_json: string | null }
      | undefined;
    const meta = parseGuildExecutionMeta(taskMetaRow?.workflow_meta_json);
    if (!meta) return { status: "ready", agent: requestedAgent, selectedBinding: null, switchedFrom: null };

    const roleKey = meta.roleKey ?? findGuildRoleForRuntimeAgent(meta.guildId, requestedAgent.id);
    if (!roleKey) return { status: "ready", agent: requestedAgent, selectedBinding: null, switchedFrom: null };

    const selectedBinding = selectGuildRuntimeBindingForRole(db as any, meta.guildId, roleKey);
    if (!selectedBinding) {
      return {
        status: "blocked",
        ...
        reason: `Guild AI execution blocked: no runtime binding for role '${roleKey}'.`,
      };
    }

    if (selectedBinding.availability_status !== "available") {
      const activeBindings = listGuildRuntimeBindings(db as any, meta.guildId).filter(
        (binding) => binding.status === "active" && binding.guild_role_key === roleKey,
      );
      const limitedCount = activeBindings.filter((binding) => binding.availability_status === "limited").length;
      return {
        status: "blocked",
        guildId: meta.guildId,
        roleKey,
        ...
        reason:
          limitedCount > 0
            ? `Guild AI execution blocked: all active '${roleKey}' runtimes are limited.`
            : `Guild AI execution blocked: no available active runtime for role '${roleKey}'.`,
      };
    }

    if (selectedBinding.runtime_agent_id === requestedAgent.id) {
      return { status: "ready", agent: requestedAgent, selectedBinding, switchedFrom: null };
    }

    const fallbackAgent = db.prepare("SELECT * FROM agents WHERE id = ?").get(selectedBinding.runtime_agent_id);
    if (!fallbackAgent) {
      return {
        status: "blocked",
        guildId: meta.guildId,
        roleKey,
        selectedBinding,
        reason: `Guild AI execution blocked: selected runtime '${selectedBinding.runtime_agent_id}' was not found.`,
      };
    }

    return { status: "ready", agent: fallbackAgent, selectedBinding, switchedFrom: requestedAgent };
  }

  function startTaskExecutionForAgent(taskId: string, execAgent: any, deptId: string | null, deptName: string): void {
    const preflight = guildExecutionPreflight(taskId, execAgent);
    if (preflight.status === "blocked") {
      const blockedAt = nowMs();
      const blockedTask = db.prepare("SELECT title, description FROM tasks WHERE id = ?").get(taskId) as
        | { title: string; description: string | null }
        | undefined;
      const blockedLang = resolveLang(blockedTask?.description ?? blockedTask?.title ?? taskId);
      appendTaskLog(taskId, "system", preflight.reason);
      if (preflight.selectedBinding?.active_limit) {
        appendTaskLog(
          taskId,
          "system",
          `Active AI limit: ${preflight.selectedBinding.api_provider_name}/${preflight.selectedBinding.model} (${preflight.selectedBinding.active_limit.limitType})`,
        );
      }
      ...
      notifyTaskStatus(taskId, blockedTask?.title ?? taskId, "pending", blockedLang);
      return;
    }
    ...
    if (preflight.switchedFrom) {
      appendTaskLog(
        taskId,
        "system",
        `Guild AI execution preflight switched from ${preflight.switchedFrom.name} to ${preflight.agent.name} for role '${preflight.selectedBinding?.guild_role_key}'.`,
      );
      broadcast("agent_status", db.prepare("SELECT * FROM agents WHERE id = ?").get(preflight.switchedFrom.id));
    }
    execAgent = preflight.agent;
