...
export function createReviewFinalizeTools(deps: CreateReviewFinalizeToolsDeps) {
  ...
  function readGuildQaMeta(raw: string | null | undefined): { guildId: string } | null {
    if (!raw) return null;
    try {
      const parsed = JSON.parse(raw) as Record<string, unknown>;
      const guildId = String(parsed.guildId ?? "").trim();
      const role = String(parsed.currentGuildRole ?? "").trim();
      return guildId && role === "qa" ? { guildId } : null;
    } catch {
      return null;
    }
  }
...
`,
  ...
  function finishReview(
    ...
    const currentTask = db
      .prepare(
        "SELECT status, department_id, source_task_id, project_id, workflow_pack_key, project_path, workflow_meta_json FROM tasks WHERE id = ?",
      )
      .get(taskId) as
      | {
          status: string;
...
`,
  ...
  `,
    ...
    const finalizeApprovedReview = () => {
      const t = nowMs();
      const latestTask = db.prepare("SELECT status, department_id, workflow_meta_json FROM tasks WHERE id = ?").get(taskId) as
        | { status: string; department_id: string | null; workflow_meta_json: string | null }
        | undefined;
      if (!latestTask || latestTask.status !== "review") return;

      const guildQa = readGuildQaMeta(latestTask.workflow_meta_json);
      if (guildQa) {
        try {
          const route = applyGuildTaskRouteDecision(db as any, {
          ...
          appendTaskLog(
            taskId,
            "system",
            `Guild AI review route: qa_pass -> ${route.status}/${route.assignedRole ?? "unassigned"}`,
          );
          endTaskExecutionSession(taskId, "guild_qa_pass");
          const updatedTask = db.prepare("SELECT * FROM tasks WHERE id = ?").get(taskId);
          broadcast("task_update", updatedTask);
          notifyTaskStatus(taskId, taskTitle, route.status, lang);
