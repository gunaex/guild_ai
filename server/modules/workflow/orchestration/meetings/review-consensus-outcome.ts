...
import { applyGuildTaskRouteDecision } from "../../../guild-ai/task-routing.ts";
...
function readGuildQaMeta(db: any, taskId: string): { guildId: string; retryCount: number } | null {
  ...
  try {
    const parsed = JSON.parse(row.workflow_meta_json) as Record<string, unknown>;
    const guildId = String(parsed.guildId ?? "").trim();
    const role = String(parsed.currentGuildRole ?? "").trim();
    const retryCount = Number(parsed.retryCount ?? 0);
    if (!guildId || role !== "qa") return null;
    return { guildId, retryCount: Number.isInteger(retryCount) && retryCount > 0 ? retryCount : 0 };
  } catch {
    return null;
  }
}
...
export async function processReviewConsensusOutcome(ctx: OutcomeContext): Promise<boolean> {
  ...
  if (needsRevision) {
    const guildQa = readGuildQaMeta(db, taskId);
    if (guildQa) {
      try {
        const route = applyGuildTaskRouteDecision(db, {
          guildId: guildQa.guildId,
          taskId,
          decision: "qa_fail",
          feedback: `Review round ${round}: QA hold signal detected.`,
          now: Date.now(),
        });
        appendTaskLog(
          taskId,
          "system",
          `Guild AI review route: qa_fail -> ${route.status}/${route.assignedRole ?? "unassigned"}`,
        );
        ...
        notifyCeo(
          pickL(
            l(
              [`[CEO OFFICE] '${taskTitle}' QA hold detected. Guild AI routed the task for retry/escalation.`],
              [`[CEO OFFICE] QA hold detected for '${taskTitle}'. Guild AI routed the task for retry/escalation.`],
              [`[CEO OFFICE] '${taskTitle}' のQA保留を検知しました。Guild AIが再試行/エスカレーションへルーティングしました。`],
              [`[CEO OFFICE] '${taskTitle}' 检测到 QA 保留。Guild AI 已路由到重试/升级。`],
            ),
            lang,
          ),
          taskId,
        ...
        appendTaskLog(
          taskId,
          "system",
          `Guild AI review route skipped: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
  }
