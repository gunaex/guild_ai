...
export function createRunCompleteHandler(deps: CreateRunCompleteHandlerDeps) {
  ...
  function readGuildIdFromWorkflowMeta(raw: string | null | undefined): string | null {
    if (!raw) return null;
    try {
      const parsed = JSON.parse(raw) as unknown;
      if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) return null;
      const guildId = String((parsed as Record<string, unknown>).guildId ?? "").trim();
      return guildId || null;
    } catch {
      return null;
    }
  ...
  function handleTaskRunComplete(taskId: string, exitCode: number): void {
    ...
    if (finalExitCode === 0) {
      ...
      const guildId = readGuildIdFromWorkflowMeta(task.workflow_meta_json);
      if (guildId && !task.source_task_id) {
        try {
          ...
          appendTaskLog(
            taskId,
            "system",
            `Guild AI lifecycle route: worker_done -> ${route.status}/${route.assignedRole ?? "unassigned"}`,
          );
          const routedTask = db.prepare("SELECT * FROM tasks WHERE id = ?").get(taskId);
          broadcast("task_update", routedTask);
          notifyTaskStatus(taskId, task.title, route.status, resolveLang(task.description ?? task.title));
          return;
        } catch (err) {
          appendTaskLog(
            taskId,
            "system",
            `Guild AI lifecycle route skipped: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }
