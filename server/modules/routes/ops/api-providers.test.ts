...
describe("api provider routes", () => {
  ...
  it("auto-configures local Ollama from the OpenAI-compatible models endpoint", async () => {
    const { app, db } = await createHarness();
    const fetchMock = vi.fn(async () =>
      jsonResponse({
        object: "list",
        data: [{ id: "llama3:latest" }, { id: "scb10x/llama3.1-typhoon2-8b-instruct:latest" }],
      }),
    );
    vi.stubGlobal("fetch", fetchMock);

    try {
      const response = await request(app).post("/api/api-providers/ollama/autoconfigure").send({}).expect(200);
      ...
      expect(response.body.models).toEqual(["llama3:latest", "scb10x/llama3.1-typhoon2-8b-instruct:latest"]);
      expect(fetchMock).toHaveBeenCalledWith(
        "http://localhost:11434/v1/models",
        expect.objectContaining({ headers: { Accept: "application/json" } }),
      );

      const row = db.prepare("SELECT name, type, base_url, models_cache FROM api_providers WHERE id = ?").get(
        response.body.id,
      ) as { name: string; type: string; base_url: string; models_cache: string };
      ...
      expect(JSON.parse(row.models_cache)).toEqual(["llama3:latest", "scb10x/llama3.1-typhoon2-8b-instruct:latest"]);
    } finally {
      db.close();
    }
  });
