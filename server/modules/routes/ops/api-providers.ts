...
async function discoverModels(type: ApiProviderType, baseUrl: string, apiKey: string): Promise<{
  models: string[];
  status?: number;
  error?: string;
}> {
  const url = buildModelsUrl(type, baseUrl, apiKey);
  const headers = buildApiProviderHeaders(type, apiKey);
  try {
    ...
    if (!resp.ok) {
      ...
      return { models: [], status: resp.status, error: errBody.slice(0, 500) };
    }
    const data = await resp.json();
    return { models: extractModelIds(type, data) };
  } catch (error) {
...
export function registerApiProviderRoutes({ app, db, nowMs }: RegisterApiProviderRoutesOptions): void {
  ...
  app.post("/api/api-providers/ollama/autoconfigure", async (req, res) => {
    const body = parseBody(req);
    const baseUrl =
      typeof body.base_url === "string" && body.base_url.trim()
        ? body.base_url.trim().replace(/\/+$/, "")
        : API_PROVIDER_PRESETS.ollama.base_url;
    const name = typeof body.name === "string" && body.name.trim() ? body.name.trim() : "Local Ollama";
    const discovery = await discoverModels("ollama", baseUrl, "");

    if (discovery.error || discovery.status) {
      return res.status(502).json({
        ok: false,
        error: discovery.error || `upstream returned ${discovery.status}`,
    ...
    const now = nowMs();
    const modelsJson = JSON.stringify(discovery.models);
    const existingRows = db.prepare("SELECT * FROM api_providers WHERE type = 'ollama' ORDER BY created_at ASC").all() as
      ApiProviderRow[];
    const existing = existingRows.find((row) => normalizeApiBaseUrl(row.base_url) === normalizeApiBaseUrl(baseUrl));

    if (existing) {
      db.prepare(
        `UPDATE api_providers
         SET name = ?, base_url = ?, api_key_enc = NULL, enabled = 1,
             models_cache = ?, models_cached_at = ?, updated_at = ?
         WHERE id = ?`,
      ).run(name, baseUrl, modelsJson, now, now, existing.id);
      return res.json({ ok: true, id: existing.id, created: false, model_count: discovery.models.length, models: discovery.models });
    }
    ...
    db.prepare(
      ...
      `,
    ).run(id, name, baseUrl, modelsJson, now, now, now);

    res.json({ ok: true, id, created: true, model_count: discovery.models.length, models: discovery.models });
  });
  ...
  app.post("/api/api-providers/:id/test", async (req, res) => {
    ...
    try {
      const discovery = await discoverModels(row.type, row.base_url, apiKey);
      if (discovery.error || discovery.status) {
        return res.json({ ok: false, status: discovery.status, error: discovery.error });
      }
      const models = mergeModelLists(officialPreset?.fallback_models ?? [], discovery.models);
      const now = nowMs();
  ...
  app.get("/api/api-providers/:id/models", async (req, res) => {
    ...
    try {
      ...
      if (discovery.error || discovery.status) {
        ...
        return res.status(502).json({ error: discovery.error || `upstream returned ${discovery.status}` });
      }
      const models = mergeModelLists(officialPreset?.fallback_models ?? [], discovery.models);
      const now = nowMs();
      db.prepare("UPDATE api_providers SET models_cache = ?, models_cached_at = ?, updated_at = ? WHERE id = ?").run(
