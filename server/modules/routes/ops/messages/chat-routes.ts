...
export function clearAgentConversation(
  ctx: ChatMessageRouteCtx,
  agentId: string,
  resetDirectChatState?: RuntimeContext["resetDirectChatState"],
): { deleted: number; clearedPendingProjectBinding: boolean } {
  const resetState = resetDirectChatState?.(agentId) as
  ...
  const result = ctx.db
    ...
    .run(agentId, agentId);
  ctx.broadcast("messages_cleared", { scope: "agent", agent_id: agentId });
  return {
    deleted: Number(result.changes),
    clearedPendingProjectBinding: Boolean(resetState?.clearedPendingProjectBinding),
  };
}
...
export function registerChatMessageRoutes(ctx: ChatMessageRouteCtx, deps: ChatMessageRouteDeps): void {
  ...
  app.delete("/api/messages", (req, res) => {
    ...
    if (agentId) {
      ...
      const result = clearAgentConversation(ctx, agentId, resetDirectChatState);
      return res.json({
        ok: true,
        deleted: result.deleted,
        cleared_pending_project_binding: result.clearedPendingProjectBinding,
      });
    }
