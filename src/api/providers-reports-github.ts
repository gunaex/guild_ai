...
export async function autoConfigureOllamaProvider(input?: {
  name?: string;
  base_url?: string;
}): Promise<{ ok: boolean; id: string; created: boolean; model_count: number; models: string[] }> {
  return post("/api/api-providers/ollama/autoconfigure", input ?? {}) as Promise<{
    ok: boolean;
    id: string;
    created: boolean;
    model_count: number;
