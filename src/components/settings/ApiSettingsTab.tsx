      ...
      <section className="space-y-4 rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 sm:p-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                void handleOllamaAutoConfigure();
              }}
              disabled={apiTesting === "ollama-autoconfigure"}
              className="rounded-lg border border-emerald-500/40 px-3 py-1 text-xs font-medium text-emerald-200 transition-colors hover:bg-emerald-500/10 disabled:opacity-50"
            >
              {apiTesting === "ollama-autoconfigure"
                ? t({ ko: "Ollama 확인 중...", en: "Checking Ollama...", ja: "Ollama確認中...", zh: "正在检查 Ollama..." })
                : t({ ko: "Ollama 자동 설정", en: "Auto Ollama", ja: "Ollama自動設定", zh: "自动 Ollama" })}
            </button>
            {!apiAddMode && (
              <button
                onClick={openAddMode}
