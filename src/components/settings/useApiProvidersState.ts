...
export function useApiProvidersState({
  ...
  const handleOllamaAutoConfigure = useCallback(async () => {
    setApiTesting("ollama-autoconfigure");
    setApiSaveError(null);
    try {
      const result = await api.autoConfigureOllamaProvider();
      setApiTestResult((prev) => ({
        ...prev,
        [result.id]: {
          ok: true,
      ...
      console.error("Ollama auto-configure failed:", error);
      setApiSaveError(error instanceof Error ? error.message : String(error));
    } finally {
      setApiTesting(null);
    }
