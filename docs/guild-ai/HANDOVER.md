...
- Internal quality-control infrastructure is implemented before any external L3 RAG backend: core stability API, deterministic eval cases/runs, memory quality lifecycle, prompt/policy versioning with checksum and active uniqueness, unified human review queue, SQLite MemoryProvider interface/provider, and `guild:doctor` core stability output. OpenJarvis/Qdrant/new Chroma runtime dependencies were intentionally not added.
- Business Continuity Test runner is implemented as `npm run guild:bct`. It covers 16 local-first stages across original Claw-Empire task behavior and Guild AI operations, then writes `artifacts/guild-bct/BCT_RESULT.md` and `artifacts/guild-bct/BCT_RESULT.json`. Latest result: PASS 16/16 stages.
